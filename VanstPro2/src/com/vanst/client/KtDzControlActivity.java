...
public class KtDzControlActivity extends Activity{
	static byte type;
	static byte index;
	byte value1;
	byte value2;
...
//		else setContentView(R.layout.kt);
		...
		refreshThread.start();
		Connect.IRControl(new byte[]{(byte)0xf1,type,index,0x00,0x00,0x00}, KtDzControlActivity.this);
	}
	@Override
...
//					kg.setBackgroundResource(R.drawable.kt_kg1);
					if(startFlag) Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte) ((Connect.rwPose == Connect.learn)?0x03:0x02),(byte)0x80,(byte)0xff}, KtDzControlActivity.this);
					else Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte) ((Connect.rwPose == Connect.learn)?0x03:0x02),(byte)0x00,(byte)0x00}, KtDzControlActivity.this);
...
//					Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte)((Connect.rwPose == Connect.learn)?0x03:0x02),value2,value3}, KtDzControlActivity.this);
					Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte)((Connect.rwPose == Connect.learn)?0x03:0x02),value2,(byte)(value3 % 32+ 0x20)}, KtDzControlActivity.this);
...
//					fs.setBackgroundResource(R.drawable.kt_fs2);
					...
					initText();
					imageView.setBackgroundResource(R.drawable.kt_learn2);
					Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte)((Connect.rwPose == Connect.learn)?0x03:0x02),value2,(byte)(value3 % 32 + 0x40)}, KtDzControlActivity.this);
...
//					down.setBackgroundResource(R.drawable.kt_down1);
				...
				if(event.getAction() == MotionEvent.ACTION_DOWN){
					...
					if(isDo){
						isDo = false;
						Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte)((Connect.rwPose == Connect.learn)?0x03:0x02),value2,(byte)(value3 % 32 + 0x80)}, KtDzControlActivity.this);
					}else{
						isDo = true;
						Connect.IRControl(new byte[]{(byte)0xf0,type,index,(byte)((Connect.rwPose == Connect.learn)?0x03:0x02),value2,(byte)(value3 % 32)}, KtDzControlActivity.this);
					}
					initText();
					imageView.setBackgroundResource(R.drawable.kt_learn2);
					kuangImageView.layout(0, 0, 0, 0);
	...
	*/
	...
	protected void onDestroy() {
		if(MainView.IRDevice != null && Connect.rwPose == Connect.learn)
			Connect.writeControl(new byte[]{(byte)0xf0,(byte)MainView.IRDevice.type,(byte)MainView.IRDevice.index,0x00,0x00,0x00}, KtDzControlActivity.this);
